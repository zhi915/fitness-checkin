...
| **v5.1.1** | v5.1.1 | ✅ 已发布 | 2026-09-11 | **热修：修复「无法创建房间 / 无法加入房间」**。三个根因：①建房时数据库触发器 `rooms_autoadd_owner` 已把房主写进成员表，客户端随后又插一次 → 撞主键（409 / 23505）→ 界面报「创建失败」但**房间其实已经建好了**；现改为「冲突即视为已入队」并忽略。⚠️ 不能改用 upsert：成员表刻意不开 UPDATE 策略，`merge-duplicates` 会被 RLS 拒（401）。②`rooms.owner` / `room_members.user_id` 都有指向 `profiles(id)` 的外键，资料没推上去时建房 / 进房会报 23503；现建房与进房前强制 `pushProfile(true)`，`join_room_by_code` 内也加了资料兜底（需在控制台跑一次 `cloudbase/patch_join_room.sql`）。③CloudBase 把 SQLSTATE 包成 `DATABASE_xxxxx`，现统一翻成中文可操作提示（RLS 拒绝 / 资料未同步 / 表结构不完整 / 跨域被拒）。**686 项测试全通过**（test_room_ui 新增 F / F2 组 7 项回归用例专门锁住这个 bug）。 |
| **v5.1.2** | v5.1.2 | ✅ 已发布 | 2026-09-11 | **组队看板自动刷新**（修复「后加入的人看得到我，我看不到后加入的人」）。根因是设计缺失而非偶发：CloudBase 的 PG 模式**没有 Realtime 订阅**，成员列表原先只在「切到组队页那一刻」和手动点刷新时拉一次，队友加入不会有任何通知。现加轮询：①进房后每 **20 秒**静默拉一次看板；②只在**页面可见**时跑，后台标签页不空转；③`visibilitychange` 回到前台**立即补拉**一次（后台期间攒的变化）；④**数据没变化就不重绘**——重绘会重建 DOM，会把用户正在输入的内容清掉；⑤队友加入时提示「有队友加入了房间 👋」；⑥退出房间 / 登出 / 云端降级时自动停表。失败完全静默，不弹错误打扰用户。**702 项测试全通过**（新增 `test_room_poll` 16 项专门覆盖这条链路）。 |
| **v5.1.3** | v5.1.3 | ✅ 已发布 | 2026-09-11 | **自定义头像 + 界面字体设置**。①**头像**：账号弹层新增头像区——**30 个预设 emoji** 任选，或**上传自己的图片**（居中裁剪 128px、白底、JPEG 0.72 压缩存 `data.avatarImage`，避免透明 PNG 变黑脸）；顶部账号入口、首页「队友今日打卡」、组队看板三处同步显示；**换 emoji 会清掉图片**（两者互斥），另有「用回表情」按钮。②**队友换头像实时可见**：头像纳入看板轮询签名（`roomSignature`），队友一换，20 秒内自动刷新。③**老库降级**：`profiles.avatar` 列不存在时（42703）自动置 `cloud.hasAvatar=false` 并**不带该列重试**，云端照常可用；联表 `select` 也同步去掉该列。④**界面字体**：「🎨 主题 / 壁纸 / 字体」里新增一栏，**无衬线（默认）/ 衬线 / 圆体 / 等宽数字** 四选一；实现方式与主题同构——只改 `<html data-font="…">` 一个属性，CSS 用 `html[data-font="x"]` 覆盖 `--font-ui` 变量即全局生效；另补 `button, input, select, textarea { font-family: inherit }`，否则切字体时输入框和按钮会纹丝不动。等宽数字只开 `tabular-nums`、不改字体，用于让卡路里 / 组数 / 日历日期对齐。**767 项测试全通过**（新增 `test_avatar` 46 项、`test_font` 19 项）。 |

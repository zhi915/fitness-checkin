  ...
  function blankData() {
    return {
      ...
      avatar: "",       // 头像 emoji（空则用默认 💪）
      avatarImage: "",  // 自定义头像图片（压缩后的 dataURL；空表示用 emoji）
      theme: "snow",     // 主题 key（空或 plain = 蓝白；night/snow/neon/warm）；默认「雪光柔白」
      wallpaper: "",     // 自定义壁纸 dataURL（空则用内置壁纸）
      font: "",          // 界面字体 key（空或 sans = 系统无衬线；serif/round/mono）
  ...
  function load() {
    try {
      ...
      if (typeof data.avatar !== "string") data.avatar = "";          // v5.1.3 头像 emoji
      if (typeof data.avatarImage !== "string") data.avatarImage = ""; // v5.1.3 自定义图片头像
      if (typeof data.theme !== "string") data.theme = "";
      if (typeof data.wallpaper !== "string") data.wallpaper = "";
      if (typeof data.font !== "string") data.font = "";   // v5.1.3 界面字体
  ...
  /* ---------- 头像（v5.1.3）----------
     两种形态：预设 emoji，或用户自己上传的压缩图片（dataURL）。
     显示优先级：图片 > emoji > 默认 💪。
     同步：profiles.emoji / profiles.avatar —— 队友那侧靠 v5.1.2 的轮询，
     20 秒内自动更新（roomSignature 里已把 avatar 指纹算进去）。 */
  var AVATAR_EMOJIS = ["💪", "🏋️", "🤸", "🧘", "🏃", "🚴",
                       "🏊", "⛹️", "🥊", "🦾", "⚡", "🔥",
                       "🐉", "🦍", "🐯", "🦊", "🐼", "🐨",
                       "🐶", "🐱", "🚀", "⭐", "🌙", "☀️",
                       "🌈", "🍎", "🥦", "🥑", "💎", "🎯"];
  function myAvatarEmoji() { return (typeof data.avatar === "string" && data.avatar) ? data.avatar : ROOM_EMOJI; }
  function myAvatarImage() { return (typeof data.avatarImage === "string") ? data.avatarImage : ""; }
  /* 把一个元素画成头像：有图片放 img，否则放 emoji 文本 */
  function paintAvatar(el, emoji, image) {
    if (!el) return;
    el.innerHTML = "";
    if (image) {
      ...
      el.textContent = emoji || ROOM_EMOJI;
    }
  }
  /* 选 emoji：同时清掉图片头像（两者互斥） */
  ...
  /* 存图片头像（传入的必须是已压缩好的 dataURL） */
  function setAvatarImage(dataUrl) {
    data.avatarImage = dataUrl || "";
    save();
    afterAvatarChange();
  }
  ...
  /* 换头像后：立刻上云 + 重渲染（自己这侧即时生效，不等云端回写） */
  function afterAvatarChange() {
    renderHeaderAccount();
    renderAvatarPicker();
    if (isCloudReady()) {
      pushProfile(true).then(function () { return loadRoomData(); })
        .then(renderRoom).catch(function () {});
    } else {
      renderRoom();
    }
  }
  ...
  function pushProfile(force) {
    ...
    var s = computeStreaks();
    var sig = myNickname() + "|" + s.current + "|" + s.total + "|" + myAvatarEmoji() + "|" +
      (myAvatarImage() ? myAvatarImage().slice(-24) : "");
    if (!force && sig === profileSig) return Promise.resolve();
    var row = SOCIAL.profileRow(cloud.user.id, myNickname(), myAvatarEmoji(), s,
      cloud.hasAvatar === false ? null : myAvatarImage());
    row.updated_at = new Date().toISOString();
    return CLOUD.upsert("profiles", [row], "id").then(function () { profileSig = sig; })
      .catch(function (e) {
        /* 老库没有 avatar 列（42703 未定义列 / PGRST204）：去掉该列重试一次，
           并记住这件事，之后都不再带 —— 没跑补丁也能正常用，只是队友看不到图片头像 */
        if (!noAvatarCol(e) || cloud.hasAvatar === false) throw e;
        cloud.hasAvatar = false;
        var row2 = SOCIAL.profileRow(cloud.user.id, myNickname(), myAvatarEmoji(), s, null);
        row2.updated_at = new Date().toISOString();
        return CLOUD.upsert("profiles", [row2], "id").then(function () { profileSig = sig; });
      });
  }
  /* 判断「profiles 表里还没有 avatar 列」 */
  function noAvatarCol(e) {
    if (!e) return false;
    var c = String(e.code || "");
    var m = String(e.message || "");
    return /42703|PGRST204|does not exist/i.test(c + " " + m) && /avatar/i.test(c + " " + m);
  }
  ...
  function loadRoomData() {
    ...
    var sel = cloud.hasAvatar === false
      ? "user_id,joined_at,profiles(nickname,emoji,streak,total_days)"
      : "user_id,joined_at,profiles(nickname,emoji,streak,total_days,avatar)";
    return CLOUD.select("room_members", {
      select: sel,
      room_id: "eq." + cloud.room.id
    }).catch(function (e) {
      /* 同上：老库没 avatar 列 → 降级重试一次，之后都用不带列的写法 */
      if (!noAvatarCol(e) || cloud.hasAvatar === false) throw e;
      cloud.hasAvatar = false;
      return CLOUD.select("room_members", {
      ...
      cloud.members = (rows || []).map(function (m) {
        var p = m.profiles || {};
        return {
          user_id: m.user_id, nickname: p.nickname, emoji: p.emoji,
          avatar: p.avatar || "", streak: p.streak, total_days: p.total_days
        };
      });
      /* 自己那一行用本地头像覆盖：刚换完头像要立刻可见，不等云端回写 */
      cloud.members.forEach(function (m) {
        if (cloud.user && m.user_id === cloud.user.id) {
          m.emoji = myAvatarEmoji();
          m.avatar = myAvatarImage();
        }
      });
  ...
  function renderHeaderAccount() {
    ...
    if (isCloudReady() && cloud.user) {
      paintAvatar(btn, myAvatarEmoji(), myAvatarImage());
  ...
  function renderTodayMates() {
    ...
    board.forEach(function (m) {
      h += '<div class="tm-item">' +
        '<div class="tm-avatar' + (m.todayChecked ? ' done' : '') + '"></div>' +
    ...
    var nodes = box.querySelectorAll(".tm-avatar");
    board.forEach(function (m, i) { if (nodes[i]) paintAvatar(nodes[i], m.emoji, m.avatar); });
  ...
  function renderRoomBody() {
    ...
    box.innerHTML = roomBoardHtml();
    /* 头像可能是图片，不能用字符串拼进 innerHTML，渲染完再逐个填充 */
    var anodes = box.querySelectorAll(".rm-avatar");
    var view = roomView();
    view.board.forEach(function (m, i) { if (anodes[i]) paintAvatar(anodes[i], m.emoji, m.avatar); });
    bindRoomBoard();
  }
  ...
  function roomBoardHtml() {
    ...
    v.board.forEach(function (m) {
      h += '<div class="room-member">' +
        '<div class="rm-avatar' + (m.todayChecked ? ' done' : '') + '"></div>' +
  ...
  function renderAvatarPicker() {
    var prev = $("avatarPreview"), grid = $("avatarGrid"), clearBtn = $("avatarClearBtn");
    if (prev) paintAvatar(prev, myAvatarEmoji(), myAvatarImage());
    var hasImg = !!myAvatarImage(), cur = myAvatarEmoji();
    if (grid) {
      grid.innerHTML = "";
      AVATAR_EMOJIS.forEach(function (e) {
        var b = document.createElement("button");
        b.type = "button";
        b.className = "avatar-opt" + (!hasImg && e === cur ? " sel" : "");
        b.textContent = e;
        b.addEventListener("click", function () { setAvatarEmoji(e); });
        grid.appendChild(b);
      });
    }
    if (clearBtn) clearBtn.hidden = !hasImg;
  }

  /* 选图 → 居中裁剪成正方形 → 缩到 128px → JPEG 压缩。
     128px / 质量 0.72 ≈ 5~8KB：界面上头像最大只显示 68px，够用；
     同时不能太大 —— 队友看板每 20 秒会拉一次 profiles，图片越大流量越夸张。
     透明 PNG 转 JPEG 会变黑，所以先铺一层白底。 */
  var AVATAR_PX = 128, AVATAR_QUALITY = 0.72, AVATAR_MAX_CHARS = 200000;
  function compressAvatar(file, cb, errCb) {
    var fr = new FileReader();
    fr.onerror = function () { if (errCb) errCb("读取图片失败"); };
    fr.onload = function () {
      var img = new Image();
      img.onerror = function () { if (errCb) errCb("这个文件不是可用的图片"); };
      img.onload = function () {
        try {
          var cv = document.createElement("canvas");
          cv.width = AVATAR_PX; cv.height = AVATAR_PX;
          var ctx = cv.getContext("2d");
          if (!ctx) { if (errCb) errCb("当前浏览器不支持图片处理"); return; }
          var side = Math.min(img.width || 1, img.height || 1);
          var sx = ((img.width || 1) - side) / 2, sy = ((img.height || 1) - side) / 2;
          ctx.fillStyle = "#ffffff";
          ctx.fillRect(0, 0, AVATAR_PX, AVATAR_PX);
          ctx.drawImage(img, sx, sy, side, side, 0, 0, AVATAR_PX, AVATAR_PX);
          cb(cv.toDataURL("image/jpeg", AVATAR_QUALITY));
        } catch (e) { if (errCb) errCb("图片处理失败"); }
      };
  ...
  function handleAvatarFile(file) {
    ...
    compressAvatar(file, function (dataUrl) {
      if (!dataUrl || dataUrl.length > AVATAR_MAX_CHARS) { toast("图片太大了，换一张小一点的"); return; }
      setAvatarImage(dataUrl);
      toast("头像已更新");
    }, function (msg) { toast(msg || "图片处理失败"); });
  }
  ...
  function bind() {
    ...
    /* 头像：上传 / 用回表情 */
    $("avatarUploadBtn").addEventListener("click", function () { var f = $("avatarFile"); if (f) f.click(); });
  ...
  function applyTheme() {
    ...
    applyFont();     // 字体与主题同批应用（备份恢复时也走这里）
  }
  /* ---------- 界面字体 ---------- */
  var DEFAULT_FONT = "sans";
  var FONTS = [
    { key: "sans",  name: "无衬线", sample: "健身 Aa", note: "系统默认，最清晰省电" },
    { key: "serif", name: "衬线",   sample: "健身 Aa", note: "宋体/思源宋体，偏纸质阅读感" },
    { key: "round", name: "圆体",   sample: "健身 Aa", note: "幼圆/丸ゴシック，柔和可爱（看设备是否自带）" },
    { key: "mono",  name: "等宽数字", sample: "128 kcal", note: "界面字体不变，只让数字等宽对齐" }
  ];
  function fontOf(key) { for (var i = 0; i < FONTS.length; i++) if (FONTS[i].key === key) return FONTS[i]; return null; }
  /* 字体只影响 <html data-font>，CSS 里用 html[data-font="x"] 覆盖 --font-ui 即可全局生效 */
  function applyFont() {
    var root = document.documentElement;
    var key = data.font || DEFAULT_FONT;
    if (key && key !== "sans" && fontOf(key)) root.setAttribute("data-font", key);
    else root.removeAttribute("data-font");
  }
  function setFont(key) {
    data.font = (fontOf(key) && key !== "sans") ? key : "sans";
    save();
    applyFont();
    renderFontGrid();
  }
  function renderFontGrid() {
    ...
    if (!grid) return;
    grid.innerHTML = "";
    var cur = data.font || DEFAULT_FONT;
    FONTS.forEach(function (f) {
      var item = document.createElement("div");
      item.className = "font-item" + (f.key === cur ? " sel" : "");
      var s = document.createElement("div");
      s.className = "font-sample sample-" + f.key;
      s.textContent = f.sample;
      var label = document.createElement("div");
      label.className = "font-label";
      label.textContent = f.name;
      item.appendChild(s);
      item.appendChild(label);
      item.addEventListener("click", function () { setFont(f.key); });
      grid.appendChild(item);
    });
    var note = $("fontNote");
    if (note) {
      var f0 = fontOf(cur) || FONTS[0];
      note.textContent = f0.note;
    }
  }
  ...
  function openTheme() { renderThemeGrid(); renderFontGrid(); updateWallpaperUI(); $("themeOverlay").hidden = false; }
